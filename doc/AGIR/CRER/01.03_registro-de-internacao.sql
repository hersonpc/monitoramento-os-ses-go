SELECT atendime.cd_convenio,
       ...
       'AGIR'                  AS "Nome Fantasia",
       mult.nr_cnes            AS "CNES",
       mult.ds_multi_empresa   AS "Razao Social da Unidade",
       'CRER'                 AS "Nome fantasia da Unidade", /* CONFORME O BANCO SELECIONADO*/
       ...
       DECODE(paciente.tp_estado_civil, 'C', 2,
                                        'S', 1,
                                        'D', 5,
                                        'G', 0,
                                        'I', 3,
       ...
       --T Transferencia
       --A, D altas
       CASE WHEN atendime.dt_alta is null then null else
       decode(mot_alt.tp_mot_alta, 'O', 1, 'T', 3, 'A', 2, 'D', 2) END AS "Motivo de saida",
...
FROM dbamv.multi_empresas mult, leito, unid_int, atendime, paciente, cidade, procedimento_sus, tipo_internacao, mot_alt, especialid
